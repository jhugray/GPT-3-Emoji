   ...
   .then(function (response) {
     if (response.ok) {
      response.json().then(function(data) {
        ...
        responsesPrompt.className = "columns is-mobile";
        responsesResponse.className = "columns is-mobile";
        resultContainer.appendChild(responsesPrompt);
        resultContainer.appendChild(responsesResponse);
        console.log(results);
        responsesPrompt.innerHTML = "<h3 class='subtitle column is-narrow'>Prompt:</h3> " + "<p class='column'>" + results[0].prompt + "</p><br />";
        responsesResponse.innerHTML = "<h3 class='subtitle column is-narrow'>Response:</h3> " + "<p class='column'>" + results[0].response + "</p><br />"; 
       });
     };
   });
};
